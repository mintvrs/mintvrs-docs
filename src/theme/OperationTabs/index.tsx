  ...
  return (
    <div className="tabs__container">
      <div className="openapi-tabs__operation-container">
        ...
        <ul ref={containerRef} role="tablist" aria-orientation="horizontal"
          className={clsx('openapi-tabs__operation-list-container', 'tabs', { 'tabs--block': block }, className)}>
          {tabValues.map(({ value, label, attributes }: TabValue) => (
            <li key={value} role="tab" tabIndex={selectedValue === value ? 0 : -1} aria-selected={selectedValue === value}
              ...
              {...(attributes as any)} className={clsx('tabs__item', 'openapi-tabs__operation-item', (attributes as any)?.className, { active: selectedValue === value })}>
              {label ?? value}
            </li>
          ))}
        </ul>
...
export default function OperationTabs(props: any): JSX.Element {
